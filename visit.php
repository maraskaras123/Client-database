        ...
        <div class="form" style="width: 700px; max-width: 700px; margin-top: 10px; margin-bottom: 0px; padding: 2px; background: #ffffb3;">
            <p>
                ...
                <? for($i = 0; $i < $visit['before_count']; $i++) {
                        echo("<img alt='photo' src='photos/{$visit['visit_id']}_before_{$i}.jpg'/>");
                    }
                    echo ("<br>"); 
                ?>
                <form action="include/UpdateInteg.php" method="post" enctype="multipart/form-data">
                    <? echo("<input type='hidden' name='visit' id='visit' value='{$visit['visit_id']}' />"); ?>
                    <input type='hidden' name='type' id='type' value='before' />
                    <input type="file" name="before[]" multiple="multiple" style="background-color: #bfbfbf; width: 500px;" accept="image/*">
                    <input type="submit" value="Submit" name="submit" style="background-color: #bfbfbf; font-weight: bold; width: 500px;" />
                </form>
            </p>
        </div>
        <div class="form" style="width: 700px; max-width: 700px; margin-top: 10px; margin-bottom: 0px; padding: 2px; background: #ffffb3;">
            <p>
                ...
                <? for($i = 0; $i < $visit['after_count']; $i++) {
                        echo("<img alt='photo' src='photos/{$visit['visit_id']}_after_{$i}.jpg'/>");
                    }
                    echo ("<br>"); 
                ?>
                <form action="include/UpdateInteg.php" method="post" enctype="multipart/form-data">
                    <? echo("<input type='hidden' name='visit' id='visit' value='{$visit['visit_id']}' />"); ?>
                    <input type='hidden' name='type' id='type' value='before' />
                    <input type="file" name="after[]" multiple="multiple" style="background-color: #bfbfbf; width: 500px;" accept="image/*">
                    <input type="submit" value="Submit" name="submit" style="background-color: #bfbfbf; font-weight: bold; width: 500px;" />
                </form>
            </p>
        </div>
